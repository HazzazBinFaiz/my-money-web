            ...
            @if (! empty($currentBook))
                <div class="border-b border-gray-100 bg-white dark:border-gray-700 dark:bg-gray-800">
                    <div class="mx-auto flex max-w-7xl items-center gap-2 px-4 py-2 sm:px-6 lg:px-8">
                        ...
                        @endif
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $currentBook->name }}</span>
                        <a href="{{ route('books.index') }}"
                           class="ms-auto text-xs text-gray-500 hover:underline dark:text-gray-400">{{ __('Book settings') }}</a>
                    </div>
                </div>
            @endif
