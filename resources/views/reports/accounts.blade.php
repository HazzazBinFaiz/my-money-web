...
<x-app-layout>
    ...
    <div class="py-6 sm:py-8" x-data="reportDetail()">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <x-range-filter :range="$range" route="reports.accounts" />

            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ __('Showing') }} <span class="font-medium text-gray-700 dark:text-gray-300">{{ $range->label() }}</span>
                · {{ trans_choice(':count account|:count accounts', $rows->count(), ['count' => $rows->count()]) }}
            </p>

            <x-chart.grouped :title="__('Income and expense per account')"
                             :subtitle="__('Transfers move money between accounts, so only their charge counts as expense.')"
                             :empty="__('No account activity in this range.')"
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse ($rows as $row)
                                <tr class="cursor-pointer transition hover:bg-gray-50/60 dark:hover:bg-gray-900/30"
                                    tabindex="0"
                                    @click="open(@js(route('reports.accounts.detail', $row['account']) . '?' . http_build_query(request()->query())))"
                                    @keydown.enter="open(@js(route('reports.accounts.detail', $row['account']) . '?' . http_build_query(request()->query())))">
                                    <td class="px-4 py-3 sm:px-6">
                                        <div class="flex items-center gap-3">
                                            @if ($row['account']->icon)
                                                <img src="{{ $row['account']->icon->url }}" alt="" class="avatar h-9 w-9">
                                            @else
                                                <span class="avatar h-9 w-9 bg-gray-100 dark:bg-gray-700"></span>
                                            @endif

                                            <span>
                                                <span class="block text-sm font-medium text-gray-900 dark:text-gray-100">{{ $row['account']->name }}</span>
                                                @if ($row['account']->type === AccountType::Contact)
                                                    <span class="block text-[11px] text-gray-500">{{ __('Contact') }}</span>
                                                @endif
                                            </span>
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-6 py-10 text-center text-sm text-gray-500">
                                        {{ __('No account activity in this range.') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
