        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl"
                 x-data="transactionForm(@js([
                    'type' => (int) old('type', TransactionType::Expense->value),
                    'amount' => old('amount', ''),
                    'charge' => old('charge', ''),
                 ]))">

                <x-ui.card>
                    <form method="POST" action="{{ route('transactions.store') }}" class="p-4 sm:p-6">
                        @csrf
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <x-ui.option-picker name="account_id" :groups="$accountGroups" :value="old('account_id')"
                            ...
                            <div>
                                ...
                                <template x-if="type === {{ TransactionType::Transfer->value }}">
                                    <x-ui.field :label="__('To account')" :error="$errors->get('to_account_id')">
                                        <x-ui.option-picker name="to_account_id" :groups="$accountGroups" :value="old('to_account_id')"
                                                            :placeholder="__('Select account')" />
                                    </x-ui.field>
                                </template>
                            </div>
                            ...
                            <div class="sm:col-span-2">
                                <x-ui.field :label="__('Note')" for="note" :error="$errors->get('note')">
                                    <textarea id="note" name="note" rows="2"
                                              ...
                                              placeholder="{{ __('Optional') }}">{{ old('note') }}</textarea>
                                </x-ui.field>
                            </div>
                        ...
                        <div class="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                            ...
                            <x-ui.button class="w-full sm:w-auto">{{ __('Create transaction') }}</x-ui.button>
                        </div>
                    </form>
                </x-ui.card>
            </div>
